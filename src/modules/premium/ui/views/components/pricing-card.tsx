...
const pricingCardVariants = cva("rounded-lg p-4 py-6 w-full", {
    variants: {
        variant: {
            ...
            highlighted: "bg-linear-to-br from-[#70c4e6] to-[#20286d] text-white",
        },
    },
    defaultVariants: {
        variant: "default",
...
interface Props extends VariantProps<typeof pricingCardIconVariants> {
    badge?: string | null;
    price: number;
    features: string[];
    title: string;
...
export const PricingCard = ({
    ...
    return (
        <div className={cn(pricingCardVariants({ variant }), className, "border")}>
            ...
            <div className="py-6">
                <Separator className="opacity-10 text-[#0d1450]" />
            </div>
            <Button
                className="w-full"
                size="lg"
